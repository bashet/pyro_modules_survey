...
// ============================================= Clients ===============================================================
    ...
    public function get_all_active_users_ajax(){
        if(! $this->current_user->id){
            redirect($this->config->base_url());
            exit();
        }
    ...
    public function get_all_not_active_users_ajax(){
        if(! $this->current_user->id){
            redirect($this->config->base_url());
            exit();
        }
    ...
    public function view_report($attempt_id = ''){
        ...
        $data['evaluators'] = get_submitted_evaluators_by_attempt_id($attempt_id);
    ...
    public function export_user($client_id){
        ...
        $sql = "select 
    				...
    				IF(attempt.report_ready=1 AND evaluator.submitted >= 3, 'Completed', IF(attempt.report_ready=1 AND evaluator.submitted!=evaluator.total, 'Closed', 'Pending')) as survey_status, 
