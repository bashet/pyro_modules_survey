...
if(! function_exists('get_programmes_details_by_client')){
	function get_programmes_details_by_client($id){
		$ci =& get_instance();
		...
		foreach ($query->result() as $item){
			$result[] = array('id' => $item->programme_id, 'name' => get_programme_name_by_id($item->programme_id));
		}
...
if( ! function_exists('generate_email_template_for_evaluator')){
    function generate_email_template_for_evaluator($data, $evaluator){
        ...
        $email_body .= '<p>Please click to the below link:</p>';
        $link        = $ci->config->base_url().'index.php/survey/evaluator_response/'.$evaluator->link_md5;
        $email_body .= '<a href="'.$link.'">'.$link.'</a>';
        $email_body .= '<br><br>Please note, you do not need to login to access this link';
        $email_body .= '<br><br><p>Kind regards,<br>';
        $email_body .= $user->first_name . ' ' . $user->last_name.'<br>';
        $email_body .= '360 Diagnostic with '.$client->name.', '. get_programme_name_by_id($participation->pid) .'</p>';
...
if( ! function_exists('get_evaluator_progress') ){
    function get_evaluator_progress($id){
        ...
        return number_format((($answered/$total_questions) * 100),0).'%';
