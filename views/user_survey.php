    ...
    if(($total_evaluators) && $total_evaluators >= 3){
        if($questions){
            ...
            foreach($questions as $q){
                ...
                echo '<tr>';
                $q_id = $q->id;
                echo '<td style="text-align:center"><input answer type="radio" name="q_answer-'.$q->id.'" value="1" '.(($my_answer->$q_id == 1)?'checked':'').'></td>';
                echo '<td style="text-align:center"><input answer type="radio" name="q_answer-'.$q->id.'" value="2" '.(($my_answer->$q_id == 2)?'checked':'').'></td>';
                echo '<td style="text-align:center"><input answer type="radio" name="q_answer-'.$q->id.'" value="3" '.(($my_answer->$q_id == 3)?'checked':'').'></td>';
                echo '<td style="text-align:center"><input answer type="radio" name="q_answer-'.$q->id.'" value="4" '.(($my_answer->$q_id == 4)?'checked':'').'></td>';
                echo '</tr>';
                echo '<tr>';
