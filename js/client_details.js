...
$(function () {
    ...
    $('#btn_add_manager').click(function (e) {
        e.preventDefault();
        $('#mdlManager').modal('show');
    });

    $('#frm_add_manager').validate({
        highlight: function(element) {
            $(element).closest('.form-group').addClass('has-error');
        },
        unhighlight: function(element) {
    ...
    $('#btn_update_client_manager').click(function () {
        //e.preventDefault();
        if($('#frm_add_manager').valid()){
            $('#mdlManager').modal('hide');
            $('#frm_add_manager').submit();
        }
    });

    $('#client_manager').on('click', '.detach_manager', function () {
        var data = this.id;
        var dataArray = data.split('-');

        swal({
            title: "Are you sure?",
            text: "You are about to detach a manager from organisation!",
            type: "warning",
            showCancelButton: true,
            confirmButtonColor: "#DD6B55",
            confirmButtonText: "Yes, detach it!",
            ...
            $.ajax({
                url: base_url + 'index.php/survey/detach_client_manager',
                type: "POST",
                data: {client_id: dataArray[0], manager_id: dataArray[1]}
            }).done(function (result) {
                setTimeout(function(){
                    swal('Successfully detached!', '', 'success');
                    window.location.reload();
